...
function isExpiredAdminToken(path, status, message) {
  if (status !== 401) return false;
  if (String(path || '').includes('/auth/login')) return false;
  if (String(path || '').includes('/auth/mark-offline')) return false;
...
function kickOutExpiredAdminSession() {
...
export async function apiRequest(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    if (isExpiredAdminToken(path, response.status, message)) {
      kickOutExpiredAdminSession();
    }
