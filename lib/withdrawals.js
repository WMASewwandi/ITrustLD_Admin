...
export function buildWithdrawalQueryParams({
  ...
  if (normalizedStatus === 'Pending' && !advancedActive) {
    return {
      ...base,
      status: 'Pending',
  ...
  if (normalizedStatus === 'Pending Authorization' && !advancedActive) {
    return {
      ...base,
  ...
  const effectiveStatus =
    advancedActive && normalizedStatus === 'Pending' ? advancedSearchIn : normalizedStatus;
...
export async function fetchWithdrawalExecutives() {
  return apiRequest('/admin/withdrawals/executives', withToken());
}
