const MONEY_COPY_RE =
  /^(?:[A-Z]{3}|[$€£¥]|Rs\.?)\s*(-?(?:[0-9]{1,3}(?:,[0-9]{3})+|[0-9]+)(?:\.[0-9]+)?)$/i;

function toPlainMoneyCopy(amount) {
  let next = String(amount).replace(/,/g, "");
  if (next.includes(".")) {
...
export function normalizeCopyText(value) {
  ...
  const money = text.match(MONEY_COPY_RE);
  return money ? toPlainMoneyCopy(money[1]) : text;
}
