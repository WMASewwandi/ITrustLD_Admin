import { loyaltyPendingCount, resolveFirstLoyaltyHref } from '@/lib/loyalty-permissions';
...
const HREF_BADGES = {
  ...
  '/loyalty?tab=orders&status=Pending%20Authorization': (c) => c?.loyalty?.orders_pending_authorization,
  '/loyalty?tab=orders&status=Pending Authorization': (c) => c?.loyalty?.orders_pending_authorization,
...
const GROUP_BADGES = {
  ...
  Orders: (c) => (c?.loyalty?.orders || 0) + (c?.loyalty?.orders_pending_authorization || 0),
...
export function buildNotificationItems(counts, permissions = []) {
  if (!counts) return [];
  ...
  const items = [
    ...
    {
      label: 'Loyalty',
      count: loyaltyPendingCount(counts, permissions),
      detail: 'pending claims/orders',
      href: resolveFirstLoyaltyHref(permissions) || '/loyalty?tab=orders&status=Pending',
