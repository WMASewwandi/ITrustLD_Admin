...
export function resolveDashboardDurationLabel(filter, year, periodLabel) {
  ...
  if (filter && filter !== DEFAULT_DASHBOARD_FILTER) return preset;
...
export async function fetchDashboardPlatforms({ filter, from, to } = {}) {
  const params = new URLSearchParams();
  // All-time list when on the default year view; otherwise match the selected period.
  if (filter && filter !== DEFAULT_DASHBOARD_FILTER) {
