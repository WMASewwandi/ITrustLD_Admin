...
export function formatDateTimeParts(value) {
  ...
  const raw = String(value || '').trim();
  if (!raw || raw === '—') {
    return { date: raw || '—', time: '', raw: value };
  }

  const match = raw.match(/^(\S+)\s+(\d{1,2}:\d{2}(?::\d{2})?)/);
  if (match) {
    return { date: match[1], time: match[2], raw: value };
  }
...
export function todayYmdColombo() {
  const parts = getColomboDateParts();
  return `${parts.year}-${pad2(parts.month)}-${pad2(parts.day)}`;
}

/** Milliseconds until the next 00:15 SL (end of ended-shift login grace). */
export function msUntilNextShiftGraceEnd(date = new Date()) {
  const parts = getColomboDateParts(date);
  const todayKick = Date.parse(
    `${parts.year}-${pad2(parts.month)}-${pad2(parts.day)}T00:15:00${SL_OFFSET}`,
  );
  const nowMs = date.getTime();
  if (Number.isFinite(todayKick) && nowMs < todayKick) {
    return todayKick - nowMs;
  }
  return todayKick + 24 * 60 * 60 * 1000 - nowMs;
}
