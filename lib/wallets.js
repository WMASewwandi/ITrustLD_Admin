import { apiFormRequest, apiRequest, getApiBaseUrl, rewritePublicAssetUrl } from '@/lib/api';
...
export function getWalletLogoUrl(wallet) {
  if (wallet?.logoUrl) return rewritePublicAssetUrl(wallet.logoUrl);
...
function buildWalletFormData(payload, logoFile) {
  ...
  formData.append('payAccountKey', String(payload.payAccountKey || '').trim());
  if (logoFile) {
...
export function mapWalletToRow(wallet) {
  ...
  return {
    ...
    payAccountKey: wallet.payAccountKey || '',
    payAccountLabel: wallet.payAccountLabel || '',
    badgeColor: '#236B6B',
