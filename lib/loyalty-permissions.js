...
/** Pending loyalty count for queues this user can actually open. */
export function loyaltyPendingCount(counts, userPermissions = []) {
  let total = 0;
  if (hasLoyaltyTabRead(userPermissions, 'orders')) {
    total += Number(counts?.loyalty?.orders || 0);
    total += Number(counts?.loyalty?.orders_pending_authorization || 0);
  }
  if (hasLoyaltyTabRead(userPermissions, 'bonus')) {
    total += Number(counts?.loyalty?.bonus || 0);
  }
  if (hasLoyaltyTabRead(userPermissions, 'vouchers')) {
    total += Number(counts?.loyalty?.vouchers || 0);
  }
  if (hasLoyaltyTabRead(userPermissions, 'gifts')) {
    total += Number(counts?.loyalty?.gifts || 0);
  }
  return total;
}
